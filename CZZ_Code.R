...
accuracy_table <- data.frame(
  ...
  SVM_accuracy = numeric(length(fold_range)) # Add SVM accuracy column
)
...
set.seed(1) # Ensure reproducibility of folds
n_folds <- 5
# Store SVM accuracy for each fold
...
# Average SVM accuracy across all folds
SVMmean_accuracy <- mean(SVMaccuracy_list)
SVMmean_accuracy
...
# Random select cross validation (不要了)
train_random <- combined_q1$features
test_random <- NULL
truth_random <- NULL
...
for (fold in 1:n_folds) {
  ...
  # Train and predict using KNN
  predsKNN_fold <- KNNCorpus(list(train_human, train_gptm), test_fold, k = 31)
  KNNaccuracy_list[fold] <- sum(predsKNN_fold == truth_fold) / length(truth_fold)
}
